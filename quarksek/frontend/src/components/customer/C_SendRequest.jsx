...
export function Send_Request_Manager() {
  ...
  const addNewMessage = () => {
    ...
    }, 800); // Hide notification after 2 seconds
  };

  //send the value to pushserver file
  ...
  const handleSend = () => {
    ...
    ws.onopen = function() {
      ...
      ws.send(JSON.stringify({ type: 'selectedOption', message: selectedOption }));
      ...
      <div className='mx-auto'>
        ...
        <div className='lg:ml-20 md:ml-10  mt-20'>
          <div className=''>
            ...
            <br />
            <div className='flex'>
              <h1 className='border border-black bg-red-800 text-white w-60 ml-20 text-center px-2     py-4 '>Waiting for Response<br />Form Team</h1>
              <DropdownChange />
            </div>
          </div>
          ...
          <div className='mt-28 flex'>
            ...
            <div className="mt-10 ">
              <button className='border flex px-2 py-1 mt-0.5 ml-2 bg-blue-900 rounded text-white hover:scale-110 transition-transform duration-300 '
                onClick={handleSend}>SEND <span className='mt-1.5 ml-1'><IoSend /></span>
              </button>
            </div>
          </div>
        </div>
